...
任务每次跑完都会在**本机**弹一条系统通知（受 Proma 设置里的通知总开关约束），这一条不需要配置、也关不掉单个任务。

除此之外可以再挑**一个远程渠道**（飞书 / 微信 / QQ 的某个聊天），用 `notify` 参数控制：

- `source`（默认）：推回**当前会话所在的那个远程聊天**。用户在微信里让你建的任务，结果就发回那个微信会话；在 QQ 群里建的就发回那个群。桌面端会话没有来源渠道，此时只有本机通知。
- `none`：不推远程，只留本机通知。

绝大多数情况不用显式传 `notify` —— 默认行为就是「从哪来推回哪去」。用户说「结果发我微信」而当前就是微信会话时，不需要做任何额外操作；用户说「别往群里发了」时用 `notify: "none"`。

用 `update_automation` 改 `notify` 时注意：在定时任务自动执行中调用 `notify: "source"` 会失败（子会话不属于任何远程聊天），要改推送目标请让用户在界面上选，或在原来的远程聊天里跟你说。

### 任务能不能"把图/文件发给我"

能不能交付文件，取决于任务的远程渠道 —— 因为完成通知只带文字，应用内会话也不会显示 Agent 产出的图片：

- **微信 / QQ 渠道**：任务运行时会拿到 `mcp__automation__send_attachment` 工具，可以把图片或文件真的发到那个聊天。用户要「每天把报表图发我微信」这类需求，在微信会话里建任务即可。
- **飞书渠道或只有本机通知**：没有交付通道。此时不要把「发给我」写进 `prompt` —— 写成「把结果保存到 xxx 并在回复里给出路径」，用户回 Proma 会话看。

用户在桌面端说「截图发给我」时，直接说明这一点并问要不要推到微信/QQ，比建一个注定发不出去的任务好。
...
- `update_automation`：修改任意字段——`name`、`prompt`、`scheduleType`/`intervalMinutes`/`timeOfDay`/`dayOfWeek`/`dayOfMonth`/`scheduledAt`（频率）、`maxRuns`（运行次数上限）、`permissionMode`（权限模式）、`sessionMode`（会话模式）、`notify`（完成通知推到哪个远程渠道）、`active`（启用/暂停）。调度相关字段变化时会自动重算下次运行时间；改 `maxRuns` 会重置已执行次数计数；把 `active` 从 false 改回 true 会重置运行配额（已完成的任务可借此再跑一轮）。**改 `maxRuns` 想让停用/已完成任务继续跑时，必须连带处理启用状态，见下文「调整 maxRuns 时连带判断启用状态」。** 自动任务执行中可省略 `id` 更新当前任务。
